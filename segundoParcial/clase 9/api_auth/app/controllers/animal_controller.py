...
#verify_jwt_in_request esta funcion se usa para veridicar la presencia y validez de un token JWY en una solicitud
#get_jwt_identity esta funcion se usa para obtener la identidad del token JWT en una solicitud protegida. proporciona acceso a la informacion del usuario autenticado a partir del token JWT, lo que permite identificar al usuario que relizó la solicitud
#functools import wraps se usa para crear decoradores que mantienen la informacion de metadatos de las funciones decoradas
